...
const ElasticSlider: React.FC<ElasticSliderProps> = ({
  ...
  return (
    <div className={`slider-container ${className}`}>
      {label && <div className="slider-label"><span>{label}</span><span className="slider-value-text">{Math.round(defaultValue)}%</span></div>}
      <Slider
...
const Slider: React.FC<SliderProps> = ({
  ...
  // Animate to default value on mount/change
  useEffect(() => {
    const controls = animate(value, defaultValue, {
      duration: 1.5,
      ease: "easeOut",
        ...
        >
          <motion.div
            style={{
              ...
              height: useTransform(scale, [1, 1.05], [8, 14]),
